...
impl HttpOps for ReqwestHttpClient {
    ...
    fn debug_info(&self) -> String {
        format!(
            ...
            auth = if self.auth_token.is_some() { "Bearer" } else { "none" },
    ...
    async fn get(&self, name: &str) -> Result<String> {
        let resp = self
            ...
            .send()
            .await
            .map_err(|e| Error::Generic { store: "HttpSecretStore", source: Box::new(e) })?;
    ...
    async fn list(&self, prefix: Option<String>) -> Result<Vec<String>> {
        ...
        let names = list_resp
            ...
            .into_iter()
            .map(|k| {
                if prefix_str.is_empty() { k } else { format!("{prefix_str}{k}") }
            })
            .collect();
        Ok(names)
    }
