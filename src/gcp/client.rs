...
impl GcpHttpClient {
    async fn token(&self) -> Result<String> {
        self.auth
            ...
            .map_err(|e| Error::Unauthenticated { source: Box::new(e) })
    }

    fn secret_path(&self, name: &str) -> String {
        format!("{}/projects/{}/secrets/{}", SECRET_MANAGER_BASE, self.project_id, name)
    }
...
impl GcpSmOps for GcpHttpClient {
    ...
    async fn get(&self, name: &str) -> Result<String> {
        ...
        let resp = self
            ...
            .send()
            .await
            .map_err(|e| Error::Generic { store: "GcpSecretManager", source: Box::new(e) })?;
        ...
        let parsed: AccessSecretResponse = resp
            .json()
            .await
            .map_err(|e| Error::Generic { store: "GcpSecretManager", source: Box::new(e) })?;
    ...
    async fn list(&self, prefix: Option<String>) -> Result<Vec<String>> {
        let token = self.token().await?;
        let base_url = format!("{}/projects/{}/secrets", SECRET_MANAGER_BASE, self.project_id);
        ...
        loop {
            ...
            let page: ListSecretsResponse = resp.json().await
                .map_err(|e| Error::Generic { store: "GcpSecretManager", source: Box::new(e) })?;

            for entry in page.secrets.unwrap_or_default() {
                let short = entry.name.split('/').last().unwrap_or(&entry.name).to_owned();
                names.push(short);
            }
