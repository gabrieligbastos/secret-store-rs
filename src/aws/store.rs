...
use super::client::{AwsSmOps, AwsSdkClient};
...
mod tests {
    use super::*;
    use super::super::client::{MockAwsSmOps, AwsSmOps as _};
    ...
    fn store_with_mock(setup: impl FnOnce(&mut MockAwsSmOps)) -> AwsSecretsManagerStore {
        ...
        setup(&mut mock);
        mock.expect_display_name().return_const("<mock-aws>".to_owned()).times(0..);
        mock.expect_debug_info().return_const("region=<mock>, endpoint=default, provider=AwsSecretsManager".to_owned()).times(0..);
    ...
    fn debug_shows_region_and_endpoint() {
        ...
        assert!(debug_str.contains("provider=AwsSecretsManager"), "debug was: {debug_str}");
    }
    ...
    async fn get_secret_calls_ops_with_correct_name() {
        ...
        assert_eq!(store.get_secret("my-secret").await.unwrap().expose_secret(), "secret-val");
    }
